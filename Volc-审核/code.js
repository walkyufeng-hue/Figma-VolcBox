...
const NativeRichTextEngine = {
  ...
  async apply(node, translatedTaggedText, styles) {
    ...
    // If text node has mixed fonts, unify to fallbackFont first so Figma allows setting characters
    if (node.fontName === figma.mixed) {
      ...
      try {
        if (state.taggedText && state.styles) {
          ...
          if (node.fontName === figma.mixed) {
            ...
            try { await figma.loadFontAsync(baseFont); } catch (e) {}
            node.fontName = baseFont;
          } else {
...
const Handlers = {
        ...
        if (node.fontName === figma.mixed) {
          ...
          for (let i = 0; i < len; i++) {
            ...
            if (f && f !== figma.mixed) {
              ...
              if (!uniqueFonts.has(k)) uniqueFonts.set(k, f);
            }
          }
          await FontManagerService.preloadAllFonts(Array.from(uniqueFonts.values()));
        } else {
          await FontManagerService.safeLoadFont(node.fontName);
        }
  ...
  'images/batch-import': async (requestId, payload) => {
    const { items, cols = 5, xGap = 24, yGap = 24 } = payload || {};
    if (!Array.isArray(items) || items.length === 0) {
      figma.notify('没有可导入的图片数据', { error: true });
      return;
    }

    const numCols = Math.max(1, parseInt(cols) || 5);
    const numXGap = Math.max(0, parseInt(xGap) || 0);
    const numYGap = Math.max(0, parseInt(yGap) || 0);
    const isSingleRow = numCols >= items.length;
    ...
    if (isSingleRow) {
      ...
      container.name = `批量导入_${items.length}张`;
      container.fills = [];
      container.layoutMode = 'HORIZONTAL';
      container.itemSpacing = numXGap;
      ...
      container.itemSpacing = numYGap;
      container.counterAxisAlignItems = 'MIN';
      container.primaryAxisSizingMode = 'AUTO';
      container.counterAxisSizingMode = 'AUTO';
    }

    container.x = Math.round(center.x);
    container.y = Math.round(center.y);

    let currentRowFrame = null;
    let rowCount = 0;
    let successCount = 0;
    const createdRects = [];
    ...
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      try {
        const uint8Data = new Uint8Array(item.bytes);
        const image = figma.createImage(uint8Data);
        const rect = figma.createRectangle();
        rect.name = item.layerName || item.name || `Image_${i + 1}`;
        const w = Math.max(1, Math.round(item.width || 100));
        const h = Math.max(1, Math.round(item.height || 100));
        rect.resize(w, h);
        rect.fills = [{
          type: 'IMAGE',
        ...
        // 初始透明度设为 0，每个图片节点入场前保持隐形，等待逐个 0~100% 渐变淡入
        rect.opacity = 0;
        ...
        if (isSingleRow) {
          ...
          if (i % numCols === 0) {
            rowCount++;
            currentRowFrame = figma.createFrame();
            currentRowFrame.name = `第 ${rowCount} 行`;
            currentRowFrame.fills = [];
            currentRowFrame.layoutMode = 'HORIZONTAL';
            currentRowFrame.itemSpacing = numXGap;
            currentRowFrame.counterAxisAlignItems = 'MIN';
            currentRowFrame.primaryAxisSizingMode = 'AUTO';
            currentRowFrame.counterAxisSizingMode = 'AUTO';
            container.appendChild(currentRowFrame);
          }
          currentRowFrame.appendChild(rect);
        }
        successCount++;
      } catch (err) {
        console.error(`Failed to create image node for ${item.name}:`, err);
      }
    }

    if (successCount === 0 || createdRects.length === 0) {
      container.remove();
      figma.notify('图片节点生成失败', { error: true });
      return;
    }

    // 1. 自动定位到所有导入图片的区域，并缩小到能看到全部的位置
    ...
    // 3. 一个一个地透明度从 0~100% 逐个渐变显示
    const sleep = ms => new Promise(r => setTimeout(r, ms));
    const totalRects = createdRects.length;

    // 根据图片总数量动态匹配最优帧率与节奏，既有清晰的“逐个渐变”仪式感，又保持流畅敏捷
    let fadeSteps = 4;
    let stepDelay = 18;
    if (totalRects <= 6) {
      fadeSteps = 5;
      stepDelay = 20; // 100ms / 张
    } else if (totalRects <= 15) {
      fadeSteps = 4;
      stepDelay = 16; // 64ms / 张
    } else if (totalRects <= 30) {
      fadeSteps = 3;
      stepDelay = 14; // 42ms / 张
    } else if (totalRects <= 60) {
      fadeSteps = 3;
      stepDelay = 10; // 30ms / 张
    } else {
      fadeSteps = 2;
    ...
    for (let i = 0; i < totalRects; i++) {
      const rect = createdRects[i];
      // 向 UI 发送当前单张载入进度
      sendToUI({
        type: 'images/import-progress',
        payload: {
          current: i + 1,
          total: totalRects,
          percent: Math.round(((i + 1) / totalRects) * 100),
          name: `(${i + 1}/${totalRects}) ${rect.name}`
        }
      });

      // 单张节点自身 0 -> 100% 透明度平滑渐变
      for (let step = 1; step <= fadeSteps; step++) {
        rect.opacity = step / fadeSteps;
        await sleep(stepDelay);
      }
      rect.opacity = 1;
    }

    figma.notify(`✨ 成功导入并排列 ${successCount} 张图片`);
    sendToUI({
      type: 'task/completed',
      requestId,
      payload: { taskId: requestId, message: `已成功导入并排版 ${successCount} 张图片` }
    });
  },
};
